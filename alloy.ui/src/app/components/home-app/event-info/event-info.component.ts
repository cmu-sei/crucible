...
export class EventInfoComponent implements OnInit, OnDestroy {
  ...
  endEvent() {
    if (this.currentEvent) {
      this.dialogService.confirm('End Event', 'Are you sure that you want to end this event?')
        .pipe(take(1))
        .subscribe(result => {
          if (result['confirm']) {
  ...
  redeployEvent() {
    if (this.currentEvent) {
      this.dialogService.confirm('Redeploy Event', 'Are you sure that you want to redeploy this event?')
        .pipe(take(1))
        .subscribe(result => {
          if (result['confirm']) {
