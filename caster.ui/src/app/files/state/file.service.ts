...
export class FileService {
  ...
  updateFileContent(fileId: string, fileContent: string): Observable<any> {
    return this.filesService.partialEditFile(fileId, { content: fileContent }).pipe(
      tap((f) => {
      this.fileUpdated(f);
    }));
  }
  ...
  private upsertFile(file: ModelFile) {
    this.fileStore.upsert(file.id, entity => {
      return {
        ...
        editorContent: file.content === null ? entity.editorContent : file.content
      };
    });
  }
  ...
  setSelectedVersionId(fileId: string, versionId: string) {
    this.fileStore.ui.upsert(fileId, entity => ({selectedVersionId: versionId}));
  }
