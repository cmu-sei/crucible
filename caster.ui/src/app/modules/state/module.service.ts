...
import {ModulesService, Module, CreateSnippetCommand, CreateModuleRepositoryCommand} from '../../generated/caster-api';
...
export class ModuleService {
  ...
  createOrUpdateModuleById(id: string): Observable<Module> {
    ...
    return this.modulesService.createTerrraformModuleFromRepository(command).pipe(
      tap((_module: Module) => {
        this.moduleStore.upsert(_module.id, {..._module});
        this.setSaved(_module.id, true);
      }),
  ...
  createVersionSnippet(createSnippetCommand: CreateSnippetCommand): Observable<string> {
    return this.modulesService.createSnippet(createSnippetCommand);
  }

  toggleSelected(id: string) {
    this.moduleStore.ui.upsert(id, entity => ({isSelected: !entity.isSelected}));
  }
