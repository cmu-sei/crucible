...
import {Breadcrumb, ProjectObjectType, ProjectQuery, ProjectService, ProjectUI, Tab} from '../../../state';
...
import {Directory, Exercise as Project, ModelFile, Module, Workspace} from 'src/app/generated/caster-api';
...
export class ProjectTabComponent implements OnInit, OnDestroy, OnChanges, CanComponentDeactivate {
  ...
  ngOnInit() {
    ...
    this.sidebarWidth$ = this.projectQuery.getRightSidebarWidth(this.project.id);

    // Subscribe when the tab changes.
    this.selectedTab$ = this.projectQuery.selectSelectedTab(this.project.id).pipe(
      ...
      tap((tab: Tab) => {
        this.breadcrumb$ = this.projectQuery.selectTabBreadcrumb(this.project.id, tab.id);
        this.updateBreadcrumb(tab);
      }),
      ...
      switchMap(([tab, entity]: [Tab, any]) => {
        switch (tab.type) {
          case ProjectObjectType.FILE:
            return iif(() => entity.content == null,
    ...
    this.projectQuery.selectOpenTabs(this.project.id).pipe(
      tap(openTabs => {
        openTabs.forEach(tab => this.updateBreadcrumb(tab));
      }),
      switchMap(openTabs => {
        const subs = openTabs.map(tab => this.projectQuery.selectTabBreadcrumb(this.project.id, tab.id).pipe(take(1)));
        return merge(subs);
      }),
  ...
  watchForChanges() {
    ...
    for (let i = this.nonactiveTabSubscriptions.length - 1; i >= 0; i--) {
      ...
      const isClosed = !this.showTabs || !this.projectUI.openTabs.some(t => t.id === naTab.id);
      if (isClosed) {
  ...
  updateBreadcrumb(tab: Tab) {
    ...
    if (obj) {
      ...
      newBreadcrumb = this.projectService.createBreadcrumb(this.project, obj, tab.type);
  ...
  canDeactivate(): Observable<boolean> | Promise<boolean> | boolean {
    ...
    const thereAreUnsavedChanges = this.projectUI.openTabs && this.projectUI.openTabs.some(tab => {
