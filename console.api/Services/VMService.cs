...
namespace S3.Vm.Console.Services
{
    public class VmService
    {
        ...
        public async Task<ManagedObjectReference> GetVm(Guid uuid)
        {
            ManagedObjectReference vmReference = null;

            if(_client != null && _sic != null)
            {
                try
                {
                    vmReference = await _client.FindByUuidAsync(_sic.searchIndex, null, uuid.ToString(), true, false);
                }
                catch (Exception ex)
                {
                    _logger.LogError(0, ex, $"Failed to get reference for " + uuid);
                    if (_client == null)
                    {
        ...
        public async Task<string> RebootVm(Guid uuid)
        {
            ...
            if (await GetPowerState(uuid) == "error")
            {
                return "error";
            }
        ...
        public async Task<VmNicOptions> GetNicOptions(Guid id, bool canManage, List<string> allowedNetworks, VirtualMachine machine)
        {
            return new VmNicOptions
            {
        ...
        public async Task<List<string>> GetVmNetworks(VirtualMachine machine, bool canManage, List<string> allowedNetworks)
        {
