...
The ultimate goal is to create a tool that gives advanced users the power and flexibility that they desire, while also allowing novice users to take advantage of complex topologies created by advanced users or create their own simple ones easily.
...
The top-level construct within project is called *directory*. A project can contain many directories. Directories contain files that make up a particular Terraform configuration, workspaces that represent a specific instance of that configuration, and sub-directories. Directories are meant to be used primarily for organization and shared resources.

Directories can contain sub-directories to create a _hierarchy_ of directories and the configuration files contained in them. When a run is created in a workspace, the files in the workspace, the workspace's directory, and all parent directories are merged together and passed to Terraform as a single configuration. This eliminates redundancy when developing many environments that are largely the same, or sharing a set of common variables or data across many configurations. 
...
- Run their plans (_Runs_ are a specific instance of a Terraform plan; explained below)
...
Directories can contain sub-directories to create a *hierarchy* of directories and the configuration files contained in them. When a run is created in a workspace, the files in the workspace, the workspace's directory, ***and all parent directories*** are merged together and passed to Terraform as a single configuration. This eliminates redundancy when developing many environments that are largely the same, or sharing a set of common variables or data across many configurations. 
<!--- The below diagram is an example of a directory hierarchy for the "MCCORC" labs. All of the labs share the same base configuration, which lives in the labs directory. However, each of the leaf directories contains its own set of variable values that adjusts which of the resources defined in the base configuration are deployed (by changing the count of certain resources). 
Additionally, labs in Course 3 have some extra resources that are not needed in any of the other labs. This is all achieved without duplicating any of the configuration in the labs directory. --->
For example, a large deployment like a Cyber Flag might have a top-level directory that defines global variables like `vlan ids` and `team ids` in use, and then sub-directories that define resources that use those variables.
...
## Dynamic Hosts (On-Demand Exercise functionality)

Caster is called by Alloy in order to deploy resources for lab or on-demand exercise (ODX) style functionality. Caster itself does not differ in its main functionality of deploying workspaces and lets Alloy handle most of the ODX functionality. 
...
A *host* consists of a name, datastore, and maximum number of virtual machines that it can support. Hosts are created and managed through the API. After a host is created, it can be assigned to an exercise. An xxercise can have many hosts. 
...
When the workspace is deleted after an ODX is finished, the host's resources will be released. If a run attempts to deploy more virtual machines than there is capacity for in the available hosts, the run will fail.
...
There are three ways that the a module can be added/refreshed to Caster:
 ...
 1. Every time that the modules list is requested, Caster API checks for updated modules in the **Terraform-Modules** group (the group ID is a Caster API setting) or any of its sub-groups.  If you add a module or version, you may have to refresh your Caster UI browser to see the change.
2. Because the Caster UI uses its internal modified date to determine if there are any "new" changes, the Caster dates could get out of sync with the GitLab dates.  In this case, an administrator can force an update of all of the modules.
3. An administrator can also individually add/refresh a module using its GitLab Project ID, whether or not it is underneath the Terraform-Modules group.
