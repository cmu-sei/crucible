...
namespace S3.Player.Api.Hubs
{
    ...
    public class ExerciseHub : Hub
    {
        ...
        public async Task Join(string idString)
        {
            ...
            var notification = await _notificationService.JoinExercise(id, _ct);
        ...
        public async Task Post(string idString, string data)
        {
            ...
            incomingData.Subject = "Exercise Notification";
            var notification = await _notificationService.PostToExercise(id, incomingData, _ct);
            if (notification.ToId != id)
            {
        ...
        {
            var id = Guid.Parse(idString);
            var notifications = await _notificationService.GetByExerciseAsync(id, _ct);
