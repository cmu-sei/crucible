...
import { Exercise, ApplicationTemplate, ApplicationService, Application } from '../../../swagger-codegen/s3.player.api';
import { DialogService } from '../../../services/dialog/dialog.service';
import { FormControl, FormGroupDirective, NgForm, Validators } from '@angular/forms';
import { ErrorStateMatcher } from '@angular/material';

@Component({
  selector: 'app-exercise-applications-select',
  templateUrl: './exercise-applications-select.component.html',
  styleUrls: ['./exercise-applications-select.component.css']
})
export class ExerciseApplicationsSelectComponent implements OnInit {

  @Input() exercise: Exercise;
  @ViewChild(ExerciseApplicationsSelectComponent, { static: false }) child;
  ...
  ngOnInit() {
    ...
    if (!this.exercise) {
      // either a team or a exercise must be provided, so roles and permissions will not be functional
      console.log('The applications select component requires either an exercise, therefore will be non-functional.');
      return;
    } else {
  ...
  /**
   * Called to update the list of apps for the exercise
   */
  updateApplications() {
    this.isLoading = true;
    this.applicationService.getExerciseApplications(this.exercise.id).subscribe(appInsts => {
 ...
 */
  ...
  /**
   * Generically saves the application for the exercise and updates the applications list
   */
  ...
  /**
   * Removes an app from the exercise
   * @param app The app to delete
   */
  deleteExerciseApplication(app: Application) {
