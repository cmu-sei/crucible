...
export class NotificationsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.notificationService.notificationHistory.subscribe(data => {
      if (data != undefined && data.length > 0) {
        this.notificationsHistory = this.notificationsHistory.concat(<Array<NotificationData>>data);
        this.notificationsHistory = this.notificationsHistory.sort((a: NotificationData, b: NotificationData) => {
    ...
    this.notificationService.viewNotification.subscribe(msg => {
      ...
      if (msg.broadcastTime != undefined) {
        ...
        if (this.pushNotificationService.permission == "granted") {
          this.pushNotificationService.create(this.notification.subject,
            {
            ...
            .subscribe(
              res => {
                if (this.notification.link != null && this.notification.link != "") {
  ...
  let maxDisplay = this.settingsService.NotificationsSettings.number_to_display;
