...
@Component({
  ...
  styleUrls: ['./add-remove-users-dialog.component.css']
})
export class AddRemoveUsersDialogComponent implements OnInit {
  ...
  public teamUserDataSource = new MatTableDataSource<TeamUser>(new Array<TeamUser>());
    ...
    this.userService.getUsers().subscribe(allUsers => {
      this.userService.getTeamUsers(team.id).subscribe(teamUsers => {
        ...
        if (tUsers.length > 0) {
          ...
          forkJoin(membershipObservable).subscribe((tmbss: Array<Array<TeamMembership>>) => {
  ...
  addUserToTeam(user: User): void {
    ...
    const index = this.teamUserDataSource.data.findIndex(u => u.user.id === user.id);
    if (index === -1) {
      this.isBusy = true;
  ...
  removeUserFromTeam(tuser: TeamUser): void {
    ...
    const index = this.teamUserDataSource.data.findIndex(u => u.user.id === tuser.user.id);
    if (index >= 0) {
      this.isBusy = true;
      this.userService.removeUserFromTeam(this.team.id, tuser.user.id).subscribe(result => {
        const tUsers = this.teamUserDataSource.data.slice(0);
  ...
  updateMembership(teamUser: TeamUser): void {
    console.log('Update Team Membership: ' + teamUser.name + '   role: ' + teamUser.teamMembership.roleId);
    const form = <TeamMembershipForm> {
      roleId: teamUser.teamMembership.roleId === '' ? null : teamUser.teamMembership.roleId
    };

    this.teamMembershipService.updateTeamMembership(teamUser.teamMembership.id, form).subscribe(result => {
